...
# upload
uploaded_files = st.sidebar.file_uploader("ファイル選択",accept_multiple_files=False)
...
if uploaded_files:
    ...
    # visualize by maatplotlib.x-axix & y-axis is selectable
    st.markdown("### 可視化　単変量")
    # set column of dataframe as option
    x = st.selectbox("X軸", df_columns)
    y = st.selectbox("Y軸", df_columns)
    # visualize by matplotlib with variables selected
    ...
    # visualize by seaborn-pairplot. can select multi values
    ...
    # 実行ボタン（なくてもよいが、その場合、処理を進めるまでエラー画面が表示される）
    ...
    if execute_pairplot:
        ...
        # steramlit上でsabornのペアプロットを表示させる
        flg = sns.pairplot(df_sns, hue="hue")
        st.pyplot(flg)
